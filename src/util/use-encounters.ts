...
export function useEncounters() {
    ...
    const parseEncounterRolls = useCallback((encounterStr: string): string => {
        let parsedStr = encounterStr;
        const regex = /(\d+)?d(\d+)([+-]\d+)?/ig;

        for (const match of Array.from(encounterStr.matchAll(regex))) {
            const [diceRoll] = match;
            const rollResult = dice.roll(diceRoll.toLowerCase());

            parsedStr = parsedStr.replace(diceRoll, rollResult);
        }
    ...
    const forceHostileEncounter = useCallback((): string => {
        ...
        if (availableHostileEncounters.length === 0) {
            setAvailableHostileEncounters(Object.values(hostileEncounters));
        }

        return parseEncounterRolls(`[Hostile] ${encounterText}`);
    }, [availableHostileEncounters, parseEncounterRolls])
    ...
    const forceNeutralEncounter = useCallback((): string => {
        ...
        if (availableNeutralEncounters.length === 0) {
            setAvailableNeutralEncounters(Object.values(neutralEncounters));
        }

        return parseEncounterRolls(`[Neutral] ${encounterText}`);
    }, [availableNeutralEncounters, parseEncounterRolls])
    ...
    const forceFeatureEncounter = useCallback((): string => {
        ...
        if (availableFeatureEncounters.length === 0) {
            setAvailableFeatureEncounters(Object.values(featureEncounters));
        }

        return parseEncounterRolls(`[Feature] ${encounterText}`);
    }, [availableFeatureEncounters, parseEncounterRolls])
